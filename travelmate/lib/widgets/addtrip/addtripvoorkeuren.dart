...
class AddTripPreferences extends StatefulWidget {
  const AddTripPreferences({
    super.key,
  });

  @override
  State<AddTripPreferences> createState() => _AddTripPreferencesState();
}

class _AddTripPreferencesState extends State<AddTripPreferences> {
...
// Functie om de waarden van de slider bij te werken
  void _updateValues(double startValue, double endValue) {
    //update in provider
    context.read<AddTripData>().setLeeftijdvoorkeur(
            ...
            children: gender.map((gender) {
              return ChoiceChip(
                ...
                onSelected: (selected) {
                  setState(() {
                    if (selected) {
                      context.read<AddTripData>().setGendervoorkeur(gender);
                    } else {
                      context.read<AddTripData>().setGendervoorkeur('');
                    }
                  });
                },
            ...
            children: afstand.map((afstand) {
              return ChoiceChip(
                ...
                onSelected: (selected) {
                  setState(() {
                    if (selected) {
                      context.read<AddTripData>().setAfstandvoorkeur(afstand);
                    } else {
                      context.read<AddTripData>().setAfstandvoorkeur('');
                    }
                  });
                },
                shape: RoundedRectangleBorder(
